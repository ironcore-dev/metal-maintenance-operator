...
KIND_CLUSTER ?= metal-maintenance-operator-test-e2e
...
docs: crd-ref-docs ## Run go generate to generate API reference documentation.
	"$(CRD_REF_DOCS)" --source-path=./api --config=./hack/api-reference/config.yaml --renderer=markdown --output-path=./docs/api-reference/api.md
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name metal-maintenance-operator-builder
	$(CONTAINER_TOOL) buildx use metal-maintenance-operator-builder
	...
	- $(CONTAINER_TOOL) buildx rm metal-maintenance-operator-builder
	rm Dockerfile.cross
...
install: manifests kustomize ## Install CRDs into the K8s cluster specified in ~/.kube/config.
	@out="$$( "$(KUSTOMIZE)" build config/crd )"; rc=$$?; \
	[ $$rc -eq 0 ] || exit $$rc; \
...
HELM_NAMESPACE ?= metal-maintenance-operator-system
## Name of the Helm release
HELM_RELEASE ?= metal-maintenance-operator
...
.PHONY: helm
helm: build-installer kubebuilder ## Regenerate dist/chart/ from dist/install.yaml (use --force for v4.16.0 breaking changes).
	"$(KUBEBUILDER)" edit --plugins=helm/v2-alpha

.PHONY: helm-force
helm-force: build-installer kubebuilder ## Regenerate dist/chart/ from dist/install.yaml (use --force for v4.16.0 breaking changes).
	"$(KUBEBUILDER)" edit --plugins=helm/v2-alpha --force

.PHONY: install-helm
install-helm: ## Install the latest version of Helm with checksum verification.
	@command -v $(HELM) >/dev/null 2>&1 || { \
		...
		TMPDIR=$$(mktemp -d) && \
		curl -fsSL "https://get.helm.sh/$${TARBALL}" -o "$${TMPDIR}/$${TARBALL}" && \
		curl -fsSL "https://get.helm.sh/$${TARBALL}.sha256sum" -o "$${TMPDIR}/$${TARBALL}.sha256sum" && \
