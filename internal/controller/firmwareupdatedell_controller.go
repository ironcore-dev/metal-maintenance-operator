...
const (
	DellFirmwareUpdateFinalizer       = "vendorconsole.ironcore.dev/firmwareupdatedell"
...
type SelectedServerListChangedDuringUpdate struct {
	missingdServersFromBaseline []string
	ServersInSpec               []string
}

func (e *SelectedServerListChangedDuringUpdate) Error() string {
	return fmt.Sprintf("selected server list changed during update: missing Server {%v}. servers selected from Spec {%v}", e.missingdServersFromBaseline, e.ServersInSpec)
}

// +kubebuilder:rbac:groups=vendorconsole.ironcore.dev,resources=firmwareupdatedells,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=vendorconsole.ironcore.dev,resources=firmwareupdatedells/status,verbs=get;update;patch
// +kubebuilder:rbac:groups=vendorconsole.ironcore.dev,resources=firmwareupdatedells/finalizers,verbs=update
...
func (r *FirmwareUpdateDELLReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	return r.reconcileExists(ctx, log, firmwareDell)
}
...
func (r *FirmwareUpdateDELLReconciler) reconcileExists(
	...
	if r.shouldDelete(log, firmwareDell) {
		...
		return r.delete(ctx, log, firmwareDell)
	}

	return r.reconcile(ctx, log, firmwareDell)
}
...
func (r *FirmwareUpdateDELLReconciler) delete(
	...
	consoleClient, err := r.getVendorConcoleClient(ctx, log, firmwareDell)
	if err != nil {
...
func (r *FirmwareUpdateDELLReconciler) cleanupServerMaintenanceReferences(
	...
	serverMaintenances, errs := r.getReferredServerMaintenances(ctx, log, firmwareDell.Spec.ServerMaintenanceRefs)
	...
	if len(finalErr) == 0 {
		...
		err := r.patchMaintenanceRequestRef(ctx, log, firmwareDell, nil)
		if err != nil {
...
func (r *FirmwareUpdateDELLReconciler) reconcile(
	...
	return r.ensureStateTransition(ctx, log, firmwareDell)
}
...
func (r *FirmwareUpdateDELLReconciler) ensureStateTransition(
	...
	case "", maintenancev1alpha1.FirmwareUpdateStatePending:
		return r.handlePendingState(ctx, log, firmwareDell, servers, serverList)
	case maintenancev1alpha1.FirmwareUpdateStateInProgress:
		return r.handleInProgressState(ctx, log, firmwareDell, servers, consoleClient)
	case maintenancev1alpha1.FirmwareUpdateStateCompleted:
		return r.handleCompletedState(ctx, log, firmwareDell, servers, consoleClient)
	case maintenancev1alpha1.FirmwareUpdateStateFailed:
		...
		return r.handleFailedState(ctx, log, firmwareDell)
	}
	return ctrl.Result{}, nil
}
...
func (r *FirmwareUpdateDELLReconciler) handlePendingState(
	...
	if len(servers) == 0 {
		log.V(1).Info("No servers found matching the Spec's selector", "Selector", firmwareDell.Spec.ServerSelector)
		err := r.updateStatus(ctx, log, firmwareDell, maintenancev1alpha1.FirmwareUpdateStateCompleted, int32(len(servers)), firmwareDell.Status.UpdateTask)
		return ctrl.Result{}, err
	}

	if len(servers) != len(serverList.Items) {
		log.V(1).Info("Some servers are not Dell", "TotalServers", len(serverList.Items))
		err := r.updateStatus(ctx, log, firmwareDell, maintenancev1alpha1.FirmwareUpdateStateFailed, int32(len(servers)), firmwareDell.Status.UpdateTask)
		return ctrl.Result{}, err
	}
	err := r.updateStatus(ctx, log, firmwareDell, maintenancev1alpha1.FirmwareUpdateStateInProgress, int32(len(servers)), firmwareDell.Status.UpdateTask)
	return ctrl.Result{}, err
}
...
func (r *FirmwareUpdateDELLReconciler) handleInProgressState(
	...
	result, err := r.handlePreUpdateTasks(ctx, log, firmwareDell, servers, consoleClient)
	if err != nil {
		...
		if errors.As(err, &serverListChangedErr) {
			...
			err := r.updateStatus(ctx, log, firmwareDell, maintenancev1alpha1.FirmwareUpdateStateFailed, firmwareDell.Status.ServerCount, firmwareDell.Status.UpdateTask)
			return ctrl.Result{}, err
		}
		return ctrl.Result{}, err
	}
	...
	if len(result.targets) == 0 {
		...
		err := r.updateStatus(ctx, log, firmwareDell, maintenancev1alpha1.FirmwareUpdateStateCompleted, firmwareDell.Status.ServerCount, firmwareDell.Status.UpdateTask)
		return ctrl.Result{}, err
	}
	...
	if len(result.baselineTask.selectedServers) != len(servers) {
		...
		if len(result.baselineTask.selectedServers) != int(firmwareDell.Status.ServerCount) {
			return ctrl.Result{}, r.updateStatus(ctx, log, firmwareDell, firmwareDell.Status.State, int32(len(result.baselineTask.selectedServers)), firmwareDell.Status.UpdateTask)
		}
	}
	...
	if len(firmwareDell.Spec.ServerMaintenanceRefs) != len(result.baselineTask.selectedServers) {
		...
		if requeue, err := r.requestMaintenanceOnServers(ctx, log, firmwareDell, result.baselineTask.selectedServers); err != nil || requeue {
			return ctrl.Result{}, err
		}
	}

	// check if the maintenance is granted
	state := r.getMaintenanceState(ctx, log, firmwareDell, result.baselineTask.selectedServers)
	if state == metalv1alpha1.ServerMaintenanceStateFailed {
		...
		return ctrl.Result{}, r.updateStatus(ctx, log, firmwareDell, maintenancev1alpha1.FirmwareUpdateStateFailed, firmwareDell.Status.ServerCount, firmwareDell.Status.UpdateTask)
	}
	...
	if firmwareDell.Status.UpdateTask == nil {
		jobDetails, err := r.performFirmwareUpdate(ctx, log, firmwareDell, consoleClient, result.baselineTask.baseline, result.catalog, result.targets)
		if err != nil {
			return ctrl.Result{}, fmt.Errorf("failed to trigger firmware update: %w", err)
		}
		err = r.updateStatus(ctx, log, firmwareDell, firmwareDell.Status.State, firmwareDell.Status.ServerCount, &maintenancev1alpha1.DellJob{Id: jobDetails.Id, Name: jobDetails.JobName})
		return ctrl.Result{}, err
	}
	...
	if !condFound || condition.Status != metav1.ConditionTrue {
		...
		err = r.updateConditions(ctx, log, firmwareDell, condition)
		return ctrl.Result{}, err
	}
...
func (r *FirmwareUpdateDELLReconciler) handleCompletedState(
	...
	if len(result.targets) > 0 {
		...
		err = r.updateStatus(ctx, log, firmwareDell, maintenancev1alpha1.FirmwareUpdateStatePending, int32(len(servers)), nil)
		if err != nil {
			return ctrl.Result{}, err
		}
		err = r.updateConditions(ctx, log, firmwareDell, nil)
		return ctrl.Result{}, err
	}
	// cleanup the serverMaintenance references if any
	if err := r.cleanupServerMaintenanceReferences(ctx, log, firmwareDell); err != nil {
...
func (r *FirmwareUpdateDELLReconciler) handlePreUpdateTasks(
	...
	var err error
	result.catalog, err = r.createOrGetCatalog(ctx, log, firmwareDell, consoleClient)
	if err != nil {
	...
	baseLineDetails, err := r.handleBaselineOperations(ctx, log, firmwareDell, consoleClient, result.catalog, servers)
	...
	result.targets, err = r.checkComplianceReport(ctx, log, consoleClient, baseLineDetails.baseline)
	if err != nil {
...
func (r *FirmwareUpdateDELLReconciler) requestMaintenanceOnServers(
	...
	if firmwareDell.Spec.ServerMaintenanceRefs != nil {
		...
		serverMaintenances, err := r.getReferredServerMaintenances(ctx, log, firmwareDell.Spec.ServerMaintenanceRefs)
		if err != nil {
	...
	err := r.patchMaintenanceRequestRef(ctx, log, firmwareDell, serverMaintenanceRefs)
	if err != nil {
...
func (r *FirmwareUpdateDELLReconciler) getMaintenanceState(
	...
	for _, server := range servers {
		if server.Status.State == metalv1alpha1.ServerStateMaintenance {
			serverMaintenanceRef, ok := r.getServerMaintenanceRefForServer(firmwareDell.Spec.ServerMaintenanceRefs, server.Spec.ServerMaintenanceRef.UID)
...
func (r *FirmwareUpdateDELLReconciler) patchMaintenanceRequestRef(
	...
	if err := r.Patch(ctx, firmwareDell, client.MergeFrom(firmwareDellBase)); err != nil {
		log.V(1).Error(err, "failed to patch FirmwareUpdateDELL with ServerMaintenances ref")
		return err
	}
...
func (r *FirmwareUpdateDELLReconciler) createOrGetCatalog(
	...
	exisitingCalalog := &ome.DellCatalogDetails{}
	for _, catalog := range catalogList {
		...
		if exisitingCalalog != nil && exisitingCalalog.Id != 0 {
			return nil, fmt.Errorf("catalogs found on OME not matching the provided spec, cannot proceed. all catalogs: %v \n created catalog %v", catalogList, exisitingCalalog)
		}
	}

	if exisitingCalalog != nil && exisitingCalalog.Id != 0 {
		completed, err := r.checkJobCompletion(ctx, console, exisitingCalalog.TaskId)
		if err != nil {
			log.V(1).Error(err, "failed to check job completion for existing catalog", "Catalog", exisitingCalalog.Id)
			return nil, err
		}
		if !completed {
			log.V(1).Info("Refreshing catalog as the job has not completed")
			err = console.RefreshCatalog(ctx, []int{exisitingCalalog.Id})
		} else {
...
func (r *FirmwareUpdateDELLReconciler) enqueueByServerRefs(
	...
	for _, firmwareDell := range firmwareDellList.Items {
		...
		serverMaintenanceRef, ok := r.getServerMaintenanceRefForServer(firmwareDell.Spec.ServerMaintenanceRefs, host.Spec.ServerMaintenanceRef.UID)
...
func (r *FirmwareUpdateDELLReconciler) getVendorConcoleClient(
	ctx context.Context,
	log logr.Logger,
	firmwareDell *maintenancev1alpha1.FirmwareUpdateDELL,
) (*ome.OME, error) {
	...
	omeSecret, err := r.getReferredSecretData(ctx, log, firmwareDell.Spec.SecretRef)
	if err != nil {
	...
	if omeSecret[maintenancev1alpha1.SecretTokenKeyName] != consoleClient.Client.Auth.Token ||
		...
		secret, err := r.getReferredSecret(ctx, log, firmwareDell.Spec.SecretRef)
		if err != nil {
			return nil, fmt.Errorf("failed to get referred secret: %w", err)
		}
