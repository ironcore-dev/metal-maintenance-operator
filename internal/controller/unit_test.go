...
var _ = Describe("shouldSkipBMC (unit)", func() {
	...
	It("returns true when target == baseline (already at target)", func() {
		run := minimalRunForUnit("b", "s")
	...
	It("returns false when target != baseline (upgrade needed)", func() {
		run := minimalRunForUnit("b", "s")
...
var _ = Describe("shouldSkipServer (unit)", func() {
	...
	It("returns true when target == baseline for that server (already at target)", func() {
		run := minimalRunForUnit("b", "s")
