...
var _ = Describe("BIOSSettings Controller", func() {
	...
	It("should successfully patch its reference to referred server", func(ctx SpecContext) {
		...
		biosSettingsV1 := &servermaintenancev1alpha1.BIOSSettings{
			ObjectMeta: metav1.ObjectMeta{
		...
		Eventually(Object(biosSettingsV1)).Should(SatisfyAll(
			HaveField("Status.State", servermaintenancev1alpha1.BIOSSettingsStateApplied),
		...
		biosSettingsV2 := &servermaintenancev1alpha1.BIOSSettings{
			ObjectMeta: metav1.ObjectMeta{
	...
	It("should request maintenance when changing power status of server, even if bios settings update does not need it", func(ctx SpecContext) {
		...
		Eventually(Object(biosSettings)).Should(SatisfyAll(
			HaveField("Status.State", servermaintenancev1alpha1.BIOSSettingsStateInProgress),
	...
	It("should wait for upgrade and reconcile when biosSettings version is correct", func(ctx SpecContext) {
		...
		Eventually(Object(biosSettings)).Should(SatisfyAll(
			HaveField("Status.State", servermaintenancev1alpha1.BIOSSettingsStatePending),
			HaveField("Status.LastAppliedTime", BeNil()),
		))
	...
	It("should allow retry using annotation", func(ctx SpecContext) {
		...
		Eventually(Object(biosSettings)).Should(SatisfyAll(
			HaveField("Status.State", servermaintenancev1alpha1.BIOSSettingsStateFailed),
	...
	It("should replace missing BIOSSettings ref in server", func(ctx SpecContext) {
		...
		biosSettings2 := &servermaintenancev1alpha1.BIOSSettings{
			ObjectMeta: metav1.ObjectMeta{
			...
			SatisfyAll(
				HaveField("Status.FlowState", ContainElement(
					SatisfyAll(
						...
						HaveField("Name", "100"),
						HaveField("State", servermaintenancev1alpha1.BIOSSettingsFlowStateFailed),
					),
				)),
			),
		)
			...
			SatisfyAll(
				HaveField("Status.FlowState", ContainElement(
					SatisfyAll(
						...
						HaveField("State", servermaintenancev1alpha1.BIOSSettingsFlowStateApplied),
					),
				)),
				HaveField("Status.FlowState", ContainElement(
					SatisfyAll(
...
func ensureBiosSettingsFlowCondition(biosSettings *servermaintenancev1alpha1.BIOSSettings) {
	By("Ensuring right number of conditions are present")
	Eventually(Object(biosSettings)).Should(
...
func ensureBiosSettingsCondition(biosSettings *servermaintenancev1alpha1.BIOSSettings, RebootNeeded bool, waitForVersionUpgrade bool) {
	commonConditions := 4
	flowCondition := 5
	...
	Eventually(Object(biosSettings)).Should(
		HaveField("Status.FlowState", WithTransform(
			func(flowStates []servermaintenancev1alpha1.BIOSSettingsFlowStatus) int {
				if len(flowStates) == 0 {
					return 0
				}
