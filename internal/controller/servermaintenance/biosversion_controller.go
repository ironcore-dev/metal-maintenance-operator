...
const (
	BIOSVersionFinalizer = "servermaintenance.metal.ironcore.dev/biosversion"
...
// +kubebuilder:rbac:groups=servermaintenance.metal.ironcore.dev,resources=biosversions,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=servermaintenance.metal.ironcore.dev,resources=biosversions/status,verbs=get;update;patch
// +kubebuilder:rbac:groups=servermaintenance.metal.ironcore.dev,resources=biosversions/finalizers,verbs=update
...
func (r *BIOSVersionReconciler) reconcileExists(ctx context.Context, biosVersion *servermaintenancev1alpha1.BIOSVersion) (ctrl.Result, error) {
...
func (r *BIOSVersionReconciler) shouldDelete(ctx context.Context, biosVersion *servermaintenancev1alpha1.BIOSVersion) (bool, error) {
	isProgressing := func() (bool, error) {
		if biosVersion.Status.State != servermaintenancev1alpha1.BIOSVersionStateInProgress {
			return false, nil
		}
...
func (r *BIOSVersionReconciler) delete(ctx context.Context, biosVersion *servermaintenancev1alpha1.BIOSVersion) (ctrl.Result, error) {
	log := ctrl.LoggerFrom(ctx)
...
func (r *BIOSVersionReconciler) cleanupServerMaintenanceReferences(ctx context.Context, biosVersion *servermaintenancev1alpha1.BIOSVersion) error {
	log := ctrl.LoggerFrom(ctx)
	if biosVersion.Spec.ServerMaintenanceRef == nil {
		return nil
	}
...
func (r *BIOSVersionReconciler) reconcile(ctx context.Context, biosVersion *servermaintenancev1alpha1.BIOSVersion) (ctrl.Result, error) {
	log := ctrl.LoggerFrom(ctx)
...
func (r *BIOSVersionReconciler) transitionState(ctx context.Context, biosVersion *servermaintenancev1alpha1.BIOSVersion) (bool, error) {
	log := ctrl.LoggerFrom(ctx)
	if biosVersion.Spec.ServerRef == nil {
	...
	case "", servermaintenancev1alpha1.BIOSVersionStatePending:
	...
	case servermaintenancev1alpha1.BIOSVersionStateInProgress:
	...
	case servermaintenancev1alpha1.BIOSVersionStateCompleted:
		return false, r.cleanup(ctx, bmcClient, biosVersion, server)
	case servermaintenancev1alpha1.BIOSVersionStateFailed:
...
func (r *BIOSVersionReconciler) handleServerMaintenance(ctx context.Context, bmcClient bmc.BMC, biosVersion *servermaintenancev1alpha1.BIOSVersion, server *metalv1alpha1.Server) (bool, error) {
	log := ctrl.LoggerFrom(ctx)
	if biosVersion.Spec.ServerMaintenanceRef == nil {
...
func (r *BIOSVersionReconciler) processInProgressState(ctx context.Context, bmcClient bmc.BMC, biosVersion *servermaintenancev1alpha1.BIOSVersion, server *metalv1alpha1.Server) (bool, error) {
	log := ctrl.LoggerFrom(ctx)
	...
	rebootPowerOffCondition, err := utils.GetCondition(r.Conditions, biosVersion.Status.Conditions, ConditionUpgradePowerOff)
	if err != nil {
		return false, err
	}
	...
	if condition.Status != metav1.ConditionTrue {
		...
		return false, r.updateStatus(ctx, biosVersion, servermaintenancev1alpha1.BIOSVersionStateCompleted, biosVersion.Status.UpgradeTask, condition)
	}
...
func (r *BIOSVersionReconciler) processFailedState(ctx context.Context, biosVersion *servermaintenancev1alpha1.BIOSVersion, server *metalv1alpha1.Server) (bool, error) {
	log := ctrl.LoggerFrom(ctx)
	if utils.ShouldRetryReconciliation(biosVersion) {
		log.V(1).Info("Retrying BIOSVersion as per annotation")
		biosVersionBase := biosVersion.DeepCopy()
		biosVersion.Status.FailedAttempts = 0
		biosVersion.Status.State = servermaintenancev1alpha1.BIOSVersionStatePending
...
func (r *BIOSVersionReconciler) cleanup(ctx context.Context, bmcClient bmc.BMC, biosVersion *servermaintenancev1alpha1.BIOSVersion, server *metalv1alpha1.Server) error {
	log := ctrl.LoggerFrom(ctx)
	currentBiosVersion, err := r.getBIOSVersionFromBMC(ctx, bmcClient, server)
	if err != nil {
		return err
	...
	if currentBiosVersion == biosVersion.Spec.Version {
		...
		return r.updateStatus(ctx, biosVersion, servermaintenancev1alpha1.BIOSVersionStateCompleted, nil, nil)
	}
	...
	if retryFailedCondition.Status == metav1.ConditionTrue {
		return r.updateStatus(ctx, biosVersion, servermaintenancev1alpha1.BIOSVersionStateInProgress, nil, retryFailedCondition)
	}
...
func (r *BIOSVersionReconciler) ensurePowerState(ctx context.Context, biosVersion *servermaintenancev1alpha1.BIOSVersion, powerState metalv1alpha1.Power) error {
	serverMaintenance, err := r.getServerMaintenanceForRef(ctx, biosVersion.Spec.ServerMaintenanceRef)
	if err != nil {
		return err
	}
...
func (r *BIOSVersionReconciler) requestServerMaintenance(ctx context.Context, biosVersion *servermaintenancev1alpha1.BIOSVersion, server *metalv1alpha1.Server) (bool, error) {
	log := ctrl.LoggerFrom(ctx)
	if biosVersion.Spec.ServerMaintenanceRef != nil {
...
func (r *BIOSVersionReconciler) checkUpdateBiosUpgradeStatus(
	...
	if taskCurrentStatus.TaskState == schemas.KilledTaskState ||
		...
		return false, r.updateStatus(ctx, biosVersion, servermaintenancev1alpha1.BIOSVersionStateFailed, upgradeCurrentTaskStatus, completedCondition)
	}
...
func (r *BIOSVersionReconciler) upgradeBIOSVersion(
	...
	if isFatal {
		...
		if errCond := r.Conditions.Update(
			...
			err := r.updateStatus(ctx, biosVersion, servermaintenancev1alpha1.BIOSVersionStateFailed, upgradeCurrentTaskStatus, issuedCondition)
			return errors.Join(errCond, err)
		}

		return r.updateStatus(ctx, biosVersion, servermaintenancev1alpha1.BIOSVersionStateFailed, upgradeCurrentTaskStatus, issuedCondition)
	}
	if err != nil {
		log.Error(err, "Failed to issue BIOS upgrade", "Version", biosVersion.Spec.Version, "Server", server.Name)
		return err
...
func (r *BIOSVersionReconciler) enqueueBiosVersionByServerRefs(ctx context.Context, obj client.Object) []ctrl.Request {
	...
	biosVersionList := &servermaintenancev1alpha1.BIOSVersionList{}
	...
	for _, biosVersion := range biosVersionList.Items {
		...
		if biosVersion.Spec.ServerMaintenanceRef == nil ||
			biosVersion.Status.State == servermaintenancev1alpha1.BIOSVersionStateCompleted ||
			biosVersion.Status.State == servermaintenancev1alpha1.BIOSVersionStateFailed {
			return nil
		}
...
func (r *BIOSVersionReconciler) enqueueBiosSettingsByBMC(ctx context.Context, obj client.Object) []ctrl.Request {
	...
	if err := clientutils.ListAndFilter(ctx, r.Client, biosVersionList, func(object client.Object) (bool, error) {
		biosVersion := object.(*servermaintenancev1alpha1.BIOSVersion)
		if biosVersion.Spec.ServerRef == nil {
			return false, nil
		}
