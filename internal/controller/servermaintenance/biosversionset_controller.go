...
const (
	BIOSVersionSetFinalizer = "servermaintenance.metal.ironcore.dev/biosversionset"
)
...
// +kubebuilder:rbac:groups=servermaintenance.metal.ironcore.dev,resources=biosversionsets,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=servermaintenance.metal.ironcore.dev,resources=biosversionsets/status,verbs=get;update;patch
// +kubebuilder:rbac:groups=servermaintenance.metal.ironcore.dev,resources=biosversionsets/finalizers,verbs=update
...
func (r *BIOSVersionSetReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	versionSet := &servermaintenancev1alpha1.BIOSVersionSet{}
...
func (r *BIOSVersionSetReconciler) reconcileExists(ctx context.Context, versionSet *servermaintenancev1alpha1.BIOSVersionSet) (ctrl.Result, error) {
...
func (r *BIOSVersionSetReconciler) delete(ctx context.Context, versionSet *servermaintenancev1alpha1.BIOSVersionSet) (ctrl.Result, error) {
...
func (r *BIOSVersionSetReconciler) handleIgnoreAnnotationPropagation(ctx context.Context, versionSet *servermaintenancev1alpha1.BIOSVersionSet) error {
	log := ctrl.LoggerFrom(ctx)
	versions, err := r.getOwnedBIOSVersions(ctx, versionSet)
	if err != nil {
		return err
	...
	return utils.HandleIgnoreAnnotationPropagation(ctx, r.Client, versionSet, versions)
}

func (r *BIOSVersionSetReconciler) handleRetryAnnotationPropagation(ctx context.Context, versionSet *servermaintenancev1alpha1.BIOSVersionSet) error {
	log := ctrl.LoggerFrom(ctx)
	versions, err := r.getOwnedBIOSVersions(ctx, versionSet)
	if err != nil {
		return err
	...
	return utils.HandleRetryAnnotationPropagation(ctx, r.Client, versionSet, versions)
}

func (r *BIOSVersionSetReconciler) reconcile(ctx context.Context, versionSet *servermaintenancev1alpha1.BIOSVersionSet) (ctrl.Result, error) {
	log := ctrl.LoggerFrom(ctx)
	...
	if utils.ShouldIgnoreReconciliation(versionSet) {
...
func (r *BIOSVersionSetReconciler) ensureBIOSVersionsForServers(ctx context.Context, servers *metalv1alpha1.ServerList, versions *servermaintenancev1alpha1.BIOSVersionList, versionSet *servermaintenancev1alpha1.BIOSVersionSet) error {
	log := ctrl.LoggerFrom(ctx)
	...
	for _, server := range servers.Items {
		if !withBiosVersion[server.Name] {
			var newBiosVersion *servermaintenancev1alpha1.BIOSVersion
			...
			if len(newBiosVersionName) > utilvalidation.DNS1123SubdomainMaxLength {
				...
				newBiosVersion = &servermaintenancev1alpha1.BIOSVersion{
...
func (r *BIOSVersionSetReconciler) deleteOrphanBIOSVersions(ctx context.Context, servers *metalv1alpha1.ServerList, versions *servermaintenancev1alpha1.BIOSVersionList) error {
	log := ctrl.LoggerFrom(ctx)
	...
	for _, biosVersion := range versions.Items {
		if !serverMap[biosVersion.Spec.ServerRef.Name] {
			if biosVersion.Status.State == servermaintenancev1alpha1.BIOSVersionStateInProgress && biosVersion.Spec.ServerMaintenanceRef != nil {
				active, err := utils.IsAnyServerMaintenanceActive(ctx, r.Client, []metalv1alpha1.ObjectReference{*biosVersion.Spec.ServerMaintenanceRef})
				if err != nil {
...
func (r *BIOSVersionSetReconciler) patchBIOSVersionFromTemplate(ctx context.Context, template *servermaintenancev1alpha1.BIOSVersionTemplate, versions *servermaintenancev1alpha1.BIOSVersionList) (bool, error) {
	log := ctrl.LoggerFrom(ctx)
	if len(versions.Items) == 0 {
	...
	var errs []error
	for _, version := range versions.Items {
		if version.Status.State == servermaintenancev1alpha1.BIOSVersionStateInProgress {
...
func (r *BIOSVersionSetReconciler) getOwnedBIOSVersionSetStatus(versionList *servermaintenancev1alpha1.BIOSVersionList) *servermaintenancev1alpha1.BIOSVersionSetStatus {
	status := &servermaintenancev1alpha1.BIOSVersionSetStatus{}
...
func (r *BIOSVersionSetReconciler) getOwnedBIOSVersions(ctx context.Context, versionSet *servermaintenancev1alpha1.BIOSVersionSet) (*servermaintenancev1alpha1.BIOSVersionList, error) {
...
func (r *BIOSVersionSetReconciler) getServersBySelector(ctx context.Context, set *servermaintenancev1alpha1.BIOSVersionSet) (*metalv1alpha1.ServerList, error) {
...
func (r *BIOSVersionSetReconciler) patchStatus(ctx context.Context, status *servermaintenancev1alpha1.BIOSVersionSetStatus, versionSet *servermaintenancev1alpha1.BIOSVersionSet) error {
...
func (r *BIOSVersionSetReconciler) enqueueByServer(ctx context.Context, obj client.Object) []ctrl.Request {
	...
	setList := &servermaintenancev1alpha1.BIOSVersionSetList{}
...
func (r *BIOSVersionSetReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		For(&servermaintenancev1alpha1.BIOSVersionSet{}).
		Owns(&servermaintenancev1alpha1.BIOSVersion{}).
