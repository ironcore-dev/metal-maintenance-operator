...
var _ = Describe("BIOSVersion Controller", func() {
	...
	It("should successfully Start and monitor Upgrade task to completion", func(ctx SpecContext) {
		...
		Eventually(Object(biosVersion)).Should(
			HaveField("Status.State", servermaintenancev1alpha1.BIOSVersionStateInProgress),
		)
...
func ensureBiosVersionConditionTransition(acc *conditionutils.Accessor, biosVersion *servermaintenancev1alpha1.BIOSVersion, server *metalv1alpha1.Server) {
	GinkgoHelper()
