...
const (
	biosSettingsFinalizer = "servermaintenance.metal.ironcore.dev/biossettings"
...
// +kubebuilder:rbac:groups=servermaintenance.metal.ironcore.dev,resources=biossettings,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=servermaintenance.metal.ironcore.dev,resources=biossettings/status,verbs=get;update;patch
// +kubebuilder:rbac:groups=servermaintenance.metal.ironcore.dev,resources=biossettings/finalizers,verbs=update
...
func (r *BIOSSettingsReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	biosSettings := &servermaintenancev1alpha1.BIOSSettings{}
...
func (r *BIOSSettingsReconciler) reconcileExists(ctx context.Context, settings *servermaintenancev1alpha1.BIOSSettings) (ctrl.Result, error) {
...
func (r *BIOSSettingsReconciler) shouldDelete(ctx context.Context, settings *servermaintenancev1alpha1.BIOSSettings) (bool, error) {
	isProgressing := func() (bool, error) {
		if settings.Status.State != servermaintenancev1alpha1.BIOSSettingsStateInProgress {
			return false, nil
		}
...
func (r *BIOSSettingsReconciler) delete(ctx context.Context, settings *servermaintenancev1alpha1.BIOSSettings) (ctrl.Result, error) {
...
func (r *BIOSSettingsReconciler) removeServerMaintenance(ctx context.Context, settings *servermaintenancev1alpha1.BIOSSettings) error {
	log := ctrl.LoggerFrom(ctx)
	if settings.Spec.ServerMaintenanceRef == nil {
		return nil
	}
...
func (r *BIOSSettingsReconciler) cleanupReferences(ctx context.Context, settings *servermaintenancev1alpha1.BIOSSettings) (err error) {
	log := ctrl.LoggerFrom(ctx)
	if settings.Spec.ServerRef == nil {
...
func (r *BIOSSettingsReconciler) reconcile(ctx context.Context, settings *servermaintenancev1alpha1.BIOSSettings) (ctrl.Result, error) {
	log := ctrl.LoggerFrom(ctx)
	...
	if err != nil {
		if apierrors.IsNotFound(err) {
			...
			if err := r.updateStatus(ctx, settings, servermaintenancev1alpha1.BIOSSettingsStatePending, nil); err != nil {
				return ctrl.Result{}, err
			}
			return ctrl.Result{}, nil
		}
...
func (r *BIOSSettingsReconciler) ensureBIOSSettingsStateTransition(ctx context.Context, bmcClient bmc.BMC, settings *servermaintenancev1alpha1.BIOSSettings, server *metalv1alpha1.Server) (ctrl.Result, error) {
	switch settings.Status.State {
	case "", servermaintenancev1alpha1.BIOSSettingsStatePending:
		return r.handleSettingPendingState(ctx, bmcClient, settings, server)
	case servermaintenancev1alpha1.BIOSSettingsStateInProgress:
		return r.handleSettingInProgressState(ctx, bmcClient, settings, server)
	case servermaintenancev1alpha1.BIOSSettingsStateApplied:
		return r.handleAppliedState(ctx, bmcClient, settings, server)
	case servermaintenancev1alpha1.BIOSSettingsStateFailed:
...
func (r *BIOSSettingsReconciler) handleSettingPendingState(ctx context.Context, bmcClient bmc.BMC, settings *servermaintenancev1alpha1.BIOSSettings, server *metalv1alpha1.Server) (ctrl.Result, error) {
	log := ctrl.LoggerFrom(ctx)
	if len(settings.Spec.SettingsFlow) == 0 {
		log.V(1).Info("Skipping BIOSSettings because no settings flow found")
		return ctrl.Result{}, r.updateStatus(ctx, settings, servermaintenancev1alpha1.BIOSSettingsStateApplied, nil)
	}
	...
	if len(pendingSettings) > 0 {
		...
		return ctrl.Result{}, r.updateStatus(ctx, settings, servermaintenancev1alpha1.BIOSSettingsStateFailed, pendingSettingStateCheckCondition)
	}
	...
	if len(duplicateName) > 0 || len(duplicateSettingsNames) > 0 {
		...
		return ctrl.Result{}, r.updateStatus(ctx, settings, servermaintenancev1alpha1.BIOSSettingsStateFailed, duplicateCheckCondition)
	}
	...
	if len(settingsDiff) == 0 && len(settings.Status.Conditions) == 0 {
		...
		return ctrl.Result{}, r.updateStatus(ctx, settings, servermaintenancev1alpha1.BIOSSettingsStateApplied, verifySettingUpdate)
	}

	var state = servermaintenancev1alpha1.BIOSSettingsStateInProgress
	var condition *metav1.Condition
	if biosVersion != settings.Spec.Version {
		...
		state = servermaintenancev1alpha1.BIOSSettingsStatePending
		condition = versionCheckCondition
	}
	return ctrl.Result{}, r.updateStatus(ctx, settings, state, condition)
}

func (r *BIOSSettingsReconciler) handleSettingInProgressState(ctx context.Context, bmcClient bmc.BMC, settings *servermaintenancev1alpha1.BIOSSettings, server *metalv1alpha1.Server) (ctrl.Result, error) {
	log := ctrl.LoggerFrom(ctx)
	...
	for _, settingsFlowItem := range settingsFlow {
		...
		if currentSettingsFlowStatus == nil {
			currentSettingsFlowStatus = &servermaintenancev1alpha1.BIOSSettingsFlowStatus{
				Priority: settingsFlowItem.Priority,
				Name:     settingsFlowItem.Name,
			}
			return ctrl.Result{}, r.updateFlowStatus(ctx, settings, servermaintenancev1alpha1.BIOSSettingsFlowStateInProgress, currentSettingsFlowStatus, nil)
		}

		// if the state is InProgress, go ahead and apply/Verify the settings
		if currentSettingsFlowStatus.State != servermaintenancev1alpha1.BIOSSettingsFlowStateInProgress {
			...
			if len(settingsDiff) == 0 {
				...
				if currentSettingsFlowStatus.State == servermaintenancev1alpha1.BIOSSettingsFlowStateApplied {
					continue
				}
				...
				return ctrl.Result{}, r.updateFlowStatus(ctx, settings, servermaintenancev1alpha1.BIOSSettingsFlowStateApplied, currentSettingsFlowStatus, verifySettingUpdate)
			}
...
func (r *BIOSSettingsReconciler) handleBMCReset(ctx context.Context, bmcClient bmc.BMC, settings *servermaintenancev1alpha1.BIOSSettings, server *metalv1alpha1.Server) (bool, error) {
	log := ctrl.LoggerFrom(ctx)
	...
	if !condFound {
		...
		if err != nil {
			...
			if errors.As(err, &invalidSettingsErr) {
				...
				err = r.updateFlowStatus(ctx, settings, servermaintenancev1alpha1.BIOSSettingsFlowStateFailed, flowStatus, inValidSettings)
				return false, errors.Join(err, r.updateStatus(ctx, settings, servermaintenancev1alpha1.BIOSSettingsStateFailed, nil))
			}
			return false, err
		}
...
func (r *BIOSSettingsReconciler) setTimeoutForAppliedSettings(ctx context.Context, settings *servermaintenancev1alpha1.BIOSSettings, flowStatus *servermaintenancev1alpha1.BIOSSettingsFlowStatus) (bool, error) {
	log := ctrl.LoggerFrom(ctx)
	...
	if timeoutCheck.Status != metav1.ConditionTrue {
		...
		if time.Now().After(startTime.Add(r.TimeoutExpiry)) {
			...
			err = r.updateFlowStatus(ctx, settings, servermaintenancev1alpha1.BIOSSettingsFlowStateFailed, flowStatus, timedOut)
			return true, errors.Join(err, r.updateStatus(ctx, settings, servermaintenancev1alpha1.BIOSSettingsStateFailed, nil))
		}
	}
	return false, nil
}
...
func (r *BIOSSettingsReconciler) verifySettingsUpdateComplete(ctx context.Context, bmcClient bmc.BMC, biosSettings *servermaintenancev1alpha1.BIOSSettings, flowItem *api.SettingsFlowItem, flowStatus *servermaintenancev1alpha1.BIOSSettingsFlowStatus, server *metalv1alpha1.Server) (bool, error) {
	...
	if verifySettingUpdate.Status != metav1.ConditionTrue {
		...
		if len(settingsDiff) == 0 {
			...
			return false, r.updateFlowStatus(ctx, biosSettings, servermaintenancev1alpha1.BIOSSettingsFlowStateApplied, flowStatus, verifySettingUpdate)
		}
...
func (r *BIOSSettingsReconciler) rebootServer(ctx context.Context, settings *servermaintenancev1alpha1.BIOSSettings, flowStatus *servermaintenancev1alpha1.BIOSSettingsFlowStatus, server *metalv1alpha1.Server) error {
	log := ctrl.LoggerFrom(ctx)
	rebootPowerOffCondition, err := utils.GetCondition(r.Conditions, flowStatus.Conditions, ConditionSettingsRebootPowerOff)
	if err != nil {
...
func (r *BIOSSettingsReconciler) applyBIOSSettings(ctx context.Context, bmcClient bmc.BMC, settings *servermaintenancev1alpha1.BIOSSettings, flowItem *api.SettingsFlowItem, flowStatus *servermaintenancev1alpha1.BIOSSettingsFlowStatus, server *metalv1alpha1.Server, issueBiosUpdate *metav1.Condition) error {
	...
	if len(pendingSettingsDiff) > 0 {
		...
		err = r.updateFlowStatus(ctx, settings, servermaintenancev1alpha1.BIOSSettingsFlowStateFailed, flowStatus, unexpectedPendingSettings)
		return errors.Join(err, r.updateStatus(ctx, settings, servermaintenancev1alpha1.BIOSSettingsStateFailed, nil))
	}
...
func (r *BIOSSettingsReconciler) ensureNoStrandedStatus(ctx context.Context, settings *servermaintenancev1alpha1.BIOSSettings) (bool, error) {
	...
	nextFlowStatuses := make([]servermaintenancev1alpha1.BIOSSettingsFlowStatus, 0)
...
func (r *BIOSSettingsReconciler) handleAppliedState(ctx context.Context, bmcClient bmc.BMC, settings *servermaintenancev1alpha1.BIOSSettings, server *metalv1alpha1.Server) (ctrl.Result, error) {
	log := ctrl.LoggerFrom(ctx)
	...
	if len(settingsDiff) > 0 {
		...
		return ctrl.Result{}, r.updateStatus(ctx, settings, servermaintenancev1alpha1.BIOSSettingsStatePending, nil)
	}
...
func (r *BIOSSettingsReconciler) handleFailedState(ctx context.Context, settings *servermaintenancev1alpha1.BIOSSettings, server *metalv1alpha1.Server) (ctrl.Result, error) {
	log := ctrl.LoggerFrom(ctx)
	if utils.ShouldRetryReconciliation(settings) {
		log.V(1).Info("Retrying reconciliation")
		biosSettingsBase := settings.DeepCopy()
		settings.Status.FailedAttempts = 0
		settings.Status.State = servermaintenancev1alpha1.BIOSSettingsStatePending
		settings.Status.FlowState = []servermaintenancev1alpha1.BIOSSettingsFlowStatus{}
...
func (r *BIOSSettingsReconciler) getBIOSVersionAndSettingsDiff(ctx context.Context, bmcClient bmc.BMC, settings *servermaintenancev1alpha1.BIOSSettings, server *metalv1alpha1.Server) (string, schemas.SettingsAttributes, error) {
	log := ctrl.LoggerFrom(ctx)
...
func (r *BIOSSettingsReconciler) isServerInMaintenance(ctx context.Context, settings *servermaintenancev1alpha1.BIOSSettings, server *metalv1alpha1.Server) bool {
	log := ctrl.LoggerFrom(ctx)
	if settings.Spec.ServerMaintenanceRef == nil {
		return false
	}
...
func (r *BIOSSettingsReconciler) requestMaintenanceForServer(ctx context.Context, settings *servermaintenancev1alpha1.BIOSSettings, server *metalv1alpha1.Server) (bool, error) {
	log := ctrl.LoggerFrom(ctx)
	if settings.Spec.ServerMaintenanceRef != nil {
...
func (r *BIOSSettingsReconciler) getBIOSSettingsByName(ctx context.Context, name string) (*servermaintenancev1alpha1.BIOSSettings, error) {
	biosSettings := &servermaintenancev1alpha1.BIOSSettings{}
...
func (r *BIOSSettingsReconciler) patchBIOSSettingsRefForServer(ctx context.Context, server *metalv1alpha1.Server, settings *servermaintenancev1alpha1.BIOSSettings) error {
	if server == nil {
		return nil
	}
...
func (r *BIOSSettingsReconciler) patchPowerState(ctx context.Context, settings *servermaintenancev1alpha1.BIOSSettings, powerState metalv1alpha1.Power) error {
	log := ctrl.LoggerFrom(ctx)
	if settings == nil {
		return fmt.Errorf("BIOSSettings is nil")
	}

	serverMaintenance, err := utils.GetServerMaintenanceForObjectReference(ctx, r.Client, settings.Spec.ServerMaintenanceRef)
	if err != nil {
		return err
	}
...
func (r *BIOSSettingsReconciler) updateFlowStatus(ctx context.Context, settings *servermaintenancev1alpha1.BIOSSettings, state servermaintenancev1alpha1.BIOSSettingsFlowState, flowStatus *servermaintenancev1alpha1.BIOSSettingsFlowStatus, condition *metav1.Condition) error {
	...
	for idx, status := range settings.Status.FlowState {
		if status.Priority == flowStatus.Priority && status.Name == flowStatus.Name {
			...
			if state == servermaintenancev1alpha1.BIOSSettingsFlowStateApplied {
				now := metav1.Now()
			...
			currentIdx = idx
			continue
		} else if state == servermaintenancev1alpha1.BIOSSettingsFlowStateInProgress &&
			status.State == servermaintenancev1alpha1.BIOSSettingsFlowStateInProgress {
			...
			settings.Status.FlowState[idx].State = servermaintenancev1alpha1.BIOSSettingsFlowStatePending
		}
	}
...
func (r *BIOSSettingsReconciler) updateStatus(ctx context.Context, settings *servermaintenancev1alpha1.BIOSSettings, state servermaintenancev1alpha1.BIOSSettingsState, condition *metav1.Condition) error {
	...
	if state == servermaintenancev1alpha1.BIOSSettingsStateApplied {
		now := metav1.Now()
	...
	} else if state == servermaintenancev1alpha1.BIOSSettingsStatePending {
...
func (r *BIOSSettingsReconciler) enqueueBiosSettingsByServerRefs(ctx context.Context, obj client.Object) []ctrl.Request {
	...
	for _, settings := range settingsList.Items {
		if settings.Spec.ServerMaintenanceRef == nil ||
			...
			settings.Status.State == servermaintenancev1alpha1.BIOSSettingsStateApplied ||
			settings.Status.State == servermaintenancev1alpha1.BIOSSettingsStateFailed {
			continue
		}
...
func (r *BIOSSettingsReconciler) enqueueBiosSettingsByBMC(ctx context.Context, obj client.Object) []ctrl.Request {
	...
	for _, server := range serverList.Items {
		...
		settings := &servermaintenancev1alpha1.BIOSSettings{}
...
func (r *BIOSSettingsReconciler) enqueueBiosSettingsByBiosVersionResource(ctx context.Context, obj client.Object) []ctrl.Request {
	version := obj.(*servermaintenancev1alpha1.BIOSVersion)
	log := ctrl.LoggerFrom(ctx).WithValues("BIOSVersion", version.Name)

	if version.Status.State != servermaintenancev1alpha1.BIOSVersionStateCompleted || version.Spec.ServerRef == nil {
		return nil
	}
	...
	for _, settings := range settingsList.Items {
		if settings.Status.State == servermaintenancev1alpha1.BIOSSettingsStateApplied ||
			settings.Status.State == servermaintenancev1alpha1.BIOSSettingsStateFailed {
			continue
		}
		reqs = append(reqs, ctrl.Request{NamespacedName: types.NamespacedName{Name: settings.Name}})
