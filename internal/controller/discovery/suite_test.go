...
var (
	...
	testRegistry *registry.RegistryServer
)
...
func TestControllers(t *testing.T) {
	...
	RunSpecs(t, "Discovery Controller Suite")
}
...
var _ = BeforeSuite(func() {
	...
	Expect(discoveryv1alpha1.AddToScheme(scheme.Scheme)).NotTo(HaveOccurred())
...
func SetupTest() *corev1.Namespace {
	...
	BeforeEach(func(ctx SpecContext) {
		...
		*ns = corev1.Namespace{
		...
		testRegistry = registry.NewServer(GinkgoLogr, ":0")
		Expect((&ServerDiscoveryReconciler{
			Client:             k8sManager.GetClient(),
			Scheme:             k8sManager.GetScheme(),
			DiscoveryNamespace: ns.Name,
			ProbeOSImage:       "probe-os",
			IgnitionProvider: func(_ context.Context, _ *metalv1alpha1.Server) ([]byte, error) {
				return []byte("ignition-data"), nil
			},
			Registry:           testRegistry,
			RegistryDataMaxAge: time.Minute,
