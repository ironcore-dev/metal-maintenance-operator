...
const (
	BMCSettingsSetFinalizer = "bmcmaintenance.metal.ironcore.dev/bmcsettingsset"
)
...
// +kubebuilder:rbac:groups=bmcmaintenance.metal.ironcore.dev,resources=bmcsettingssets,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=bmcmaintenance.metal.ironcore.dev,resources=bmcsettingssets/status,verbs=get;update;patch
// +kubebuilder:rbac:groups=bmcmaintenance.metal.ironcore.dev,resources=bmcsettingssets/finalizers,verbs=update
...
func (r *BMCSettingsSetReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	bmcSettingsSet := &bmcmaintenancev1alpha1.BMCSettingsSet{}
...
func (r *BMCSettingsSetReconciler) getOwnedBMCSettings(
	ctx context.Context,
	bmcSettingsSet *bmcmaintenancev1alpha1.BMCSettingsSet,
) (*bmcmaintenancev1alpha1.BMCSettingsList, error) {
...
func (r *BMCSettingsSetReconciler) getOwnedBMCSettingsSetStatus(
	bmcSettingsList *bmcmaintenancev1alpha1.BMCSettingsList,
) *bmcmaintenancev1alpha1.BMCSettingsSetStatus {
	currentStatus := &bmcmaintenancev1alpha1.BMCSettingsSetStatus{}
	...
	for _, bmc := range bmcList.Items {
		if _, ok := bmcWithSettings[bmc.Name]; !ok {
			if bmc.Spec.BMCSettingRef != nil {
				if err := r.Get(ctx, client.ObjectKey{Name: bmc.Spec.BMCSettingRef.Name}, &bmcmaintenancev1alpha1.BMCSettings{}); err != nil {
			...
			var newBMCSettings *bmcmaintenancev1alpha1.BMCSettings
			...
			if len(newBMCSettingsName) > utilvalidation.DNS1123SubdomainMaxLength {
				...
				newBMCSettings = &bmcmaintenancev1alpha1.BMCSettings{
...
func (r *BMCSettingsSetReconciler) deleteOrphanedBMCSettings(
	...
	for _, bmcSettings := range bmcSettingsList.Items {
		if _, ok := bmcMap[bmcSettings.Spec.BMCRef.Name]; !ok {
			if bmcSettings.Status.State == bmcmaintenancev1alpha1.BMCSettingsStateInProgress && len(bmcSettings.Spec.ServerMaintenanceRefs) > 0 {
				refs := make([]metalv1alpha1.ObjectReference, 0, len(bmcSettings.Spec.ServerMaintenanceRefs))
				for _, item := range bmcSettings.Spec.ServerMaintenanceRefs {
					if item.ServerMaintenanceRef != nil {
						refs = append(refs, *item.ServerMaintenanceRef)
...
func (r *BMCSettingsSetReconciler) patchBMCSettingsFromTemplate(
	...
	var errs []error
	for _, bmcSettings := range bmcSettingsList.Items {
		if bmcSettings.Status.State == bmcmaintenancev1alpha1.BMCSettingsStateInProgress {
...
func (r *BMCSettingsSetReconciler) handleRetryAnnotationPropagation(ctx context.Context, set *bmcmaintenancev1alpha1.BMCSettingsSet) error {
	log := ctrl.LoggerFrom(ctx)
...
func (r *BMCSettingsSetReconciler) enqueueByBMC(
	...
	bmcSettingsSetList := &bmcmaintenancev1alpha1.BMCSettingsSetList{}
...
func (r *BMCSettingsSetReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		For(&bmcmaintenancev1alpha1.BMCSettingsSet{}).
		Owns(&bmcmaintenancev1alpha1.BMCSettings{}).
		Watches(
