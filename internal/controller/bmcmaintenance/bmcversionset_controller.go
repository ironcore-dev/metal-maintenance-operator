...
const (
	BMCVersionSetFinalizer = "bmcmaintenance.metal.ironcore.dev/bmcversionset"
)
...
// +kubebuilder:rbac:groups=bmcmaintenance.metal.ironcore.dev,resources=bmcversionsets,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=bmcmaintenance.metal.ironcore.dev,resources=bmcversionsets/status,verbs=get;update;patch
// +kubebuilder:rbac:groups=bmcmaintenance.metal.ironcore.dev,resources=bmcversionsets/finalizers,verbs=update
...
func (r *BMCVersionSetReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	bmcVersionSet := &bmcmaintenancev1alpha1.BMCVersionSet{}
...
func (r *BMCVersionSetReconciler) handleIgnoreAnnotationPropagation(
	...
	return utils.HandleIgnoreAnnotationPropagation(ctx, r.Client, bmcVersionSet, ownedBMCVersions)
}
...
func (r *BMCVersionSetReconciler) handleRetryAnnotationPropagation(
	...
	return utils.HandleRetryAnnotationPropagation(ctx, r.Client, bmcVersionSet, ownedBMCVersion)
}
...
func (r *BMCVersionSetReconciler) reconcile(
	...
	if utils.ShouldIgnoreReconciliation(bmcVersionSet) {
...
func (r *BMCVersionSetReconciler) createMissingBMCVersions(
	...
	for _, bmc := range bmcList.Items {
		if _, ok := BMCWithBMCVersion[bmc.Name]; !ok {
			newBMCVersion := &bmcmaintenancev1alpha1.BMCVersion{
...
func (r *BMCVersionSetReconciler) deleteOrphanBMCVersions(
	...
	for _, bmcVersion := range bmcVersionList.Items {
		if _, ok := bmcMap[bmcVersion.Spec.BMCRef.Name]; !ok {
			if bmcVersion.Status.State == bmcmaintenancev1alpha1.BMCVersionStateInProgress && len(bmcVersion.Spec.ServerMaintenanceRefs) > 0 {
...
func (r *BMCVersionSetReconciler) patchBMCVersionfromTemplate(
	...
	var errs []error
	for _, bmcVersion := range bmcVersionList.Items {
		if bmcVersion.Status.State == bmcmaintenancev1alpha1.BMCVersionStateInProgress && bmcVersion.Status.UpgradeTask != nil {
...
func (r *BMCVersionSetReconciler) getOwnedBMCVersionSetStatus(
	bmcVersionList *bmcmaintenancev1alpha1.BMCVersionList,
) *bmcmaintenancev1alpha1.BMCVersionSetStatus {
	currentStatus := &bmcmaintenancev1alpha1.BMCVersionSetStatus{}
...
func (r *BMCVersionSetReconciler) getOwnedBMCVersions(
	ctx context.Context,
	bmcVersionSet *bmcmaintenancev1alpha1.BMCVersionSet,
) (*bmcmaintenancev1alpha1.BMCVersionList, error) {
	bmcVersionList := &bmcmaintenancev1alpha1.BMCVersionList{}
...
func (r *BMCVersionSetReconciler) enqueueByBMC(ctx context.Context, obj client.Object) []ctrl.Request {
	...
	bmcVersionSetList := &bmcmaintenancev1alpha1.BMCVersionSetList{}
...
func (r *BMCVersionSetReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		For(&bmcmaintenancev1alpha1.BMCVersionSet{}).
		Watches(
			&bmcmaintenancev1alpha1.BMCVersion{},
			handler.EnqueueRequestForOwner(r.Scheme, r.RESTMapper(), &bmcmaintenancev1alpha1.BMCVersionSet{}),
			builder.WithPredicates(
				predicate.Funcs{
					...
					UpdateFunc: func(e event.UpdateEvent) bool {
						return utils.EnqueueFromChildObjUpdatesExceptAnnotation(e)
					},
