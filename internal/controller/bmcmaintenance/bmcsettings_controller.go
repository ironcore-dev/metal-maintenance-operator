...
const (
	BMCSettingFinalizer = "bmcmaintenance.metal.ironcore.dev/bmcsettings"
...
// +kubebuilder:rbac:groups=bmcmaintenance.metal.ironcore.dev,resources=bmcsettings,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=bmcmaintenance.metal.ironcore.dev,resources=bmcsettings/status,verbs=get;update;patch
// +kubebuilder:rbac:groups=bmcmaintenance.metal.ironcore.dev,resources=bmcsettings/finalizers,verbs=update
...
func (r *BMCSettingsReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	settings := &bmcmaintenancev1alpha1.BMCSettings{}
...
func (r *BMCSettingsReconciler) reconcileExists(ctx context.Context, settings *bmcmaintenancev1alpha1.BMCSettings) (ctrl.Result, error) {
	log := ctrl.LoggerFrom(ctx)
...
func (r *BMCSettingsReconciler) shouldDelete(ctx context.Context, bmcSetting *bmcmaintenancev1alpha1.BMCSettings) (bool, error) {
	isProgressing := func() (bool, error) {
		if bmcSetting.Status.State != bmcmaintenancev1alpha1.BMCSettingsStateInProgress {
			return false, nil
		}
...
func (r *BMCSettingsReconciler) delete(ctx context.Context, settings *bmcmaintenancev1alpha1.BMCSettings) (ctrl.Result, error) {
	log := ctrl.LoggerFrom(ctx)
...
func (r *BMCSettingsReconciler) cleanupServerMaintenanceReferences(ctx context.Context, settings *bmcmaintenancev1alpha1.BMCSettings) error {
	log := ctrl.LoggerFrom(ctx)
...
func (r *BMCSettingsReconciler) cleanupReferences(ctx context.Context, settings *bmcmaintenancev1alpha1.BMCSettings) error {
...
func (r *BMCSettingsReconciler) reconcile(ctx context.Context, settings *bmcmaintenancev1alpha1.BMCSettings) (ctrl.Result, error) {
	log := ctrl.LoggerFrom(ctx)
...
func (r *BMCSettingsReconciler) ensureBMCSettingsMaintenanceStateTransition(ctx context.Context, settings *bmcmaintenancev1alpha1.BMCSettings, bmcObj *metalv1alpha1.BMC) (ctrl.Result, error) {
	...
	case "", bmcmaintenancev1alpha1.BMCSettingsStatePending:
		...
		var state = bmcmaintenancev1alpha1.BMCSettingsStateInProgress
		...
		if bmcObj.Status.FirmwareVersion != settings.Spec.Version {
			...
			state = bmcmaintenancev1alpha1.BMCSettingsStatePending
	...
	case bmcmaintenancev1alpha1.BMCSettingsStateInProgress:
		return r.handleSettingInProgressState(ctx, settings, bmcObj, bmcClient)
	case bmcmaintenancev1alpha1.BMCSettingsStateApplied:
		return ctrl.Result{}, r.handleSettingAppliedState(ctx, settings, bmcObj, bmcClient)
	case bmcmaintenancev1alpha1.BMCSettingsStateFailed:
...
func (r *BMCSettingsReconciler) handleSettingInProgressState(ctx context.Context, settings *bmcmaintenancev1alpha1.BMCSettings, bmcObj *metalv1alpha1.BMC, bmcClient bmc.BMC) (ctrl.Result, error) {
	log := ctrl.LoggerFrom(ctx)
	...
	if len(settingsDiff) == 0 {
		return ctrl.Result{}, r.updateBMCSettingsStatus(ctx, settings, bmcmaintenancev1alpha1.BMCSettingsStateApplied, nil)
	}
...
func (r *BMCSettingsReconciler) updateSettingsAndVerify(ctx context.Context, settings *bmcmaintenancev1alpha1.BMCSettings, bmcObj *metalv1alpha1.BMC, settingsDiff schemas.SettingsAttributes, bmcClient bmc.BMC) (ctrl.Result, error) {
	log := ctrl.LoggerFrom(ctx)
		...
		if len(pendingAttr) == 0 {
			...
			if err != nil {
				...
				if errors.As(err, &invalidSettingsErr) {
					...
					err := r.updateBMCSettingsStatus(ctx, settings, bmcmaintenancev1alpha1.BMCSettingsStateFailed, inValidSettings)
					return ctrl.Result{}, err
				}
	...
	if len(settingsDiff) == 0 {
		...
		return ctrl.Result{}, r.updateBMCSettingsStatus(ctx, settings, bmcmaintenancev1alpha1.BMCSettingsStateApplied, BMCSettingsVerifiedCondition)
	}
...
func (r *BMCSettingsReconciler) persistApplyCycleConditions(ctx context.Context, settings *bmcmaintenancev1alpha1.BMCSettings, changesIssued *metav1.Condition, resetBMCReq bool) error {
	log := ctrl.LoggerFrom(ctx)
...
func (r *BMCSettingsReconciler) handleSettingAppliedState(ctx context.Context, settings *bmcmaintenancev1alpha1.BMCSettings, bmcObj *metalv1alpha1.BMC, bmcClient bmc.BMC) error {
	log := ctrl.LoggerFrom(ctx)
...
func (r *BMCSettingsReconciler) handleBMCPowerState(
	...
	bmcSetting *bmcmaintenancev1alpha1.BMCSettings,
) (bool, error) {
	log := ctrl.LoggerFrom(ctx)
	...
	default:
		...
		err = r.updateBMCSettingsStatus(ctx, bmcSetting, bmcmaintenancev1alpha1.BMCSettingsStateFailed, BMCPoweredOffCond)
		return true, err
	}
	return false, nil
}
...
func (r *BMCSettingsReconciler) handleFailedState(ctx context.Context, settings *bmcmaintenancev1alpha1.BMCSettings, bmcObj *metalv1alpha1.BMC) error {
	log := ctrl.LoggerFrom(ctx)
	if utils.ShouldRetryReconciliation(settings) {
		log.V(1).Info("Retrying BMCSettings as per annotation")
		settingsBase := settings.DeepCopy()
		settings.Status.FailedAttempts = 0
		settings.Status.State = bmcmaintenancev1alpha1.BMCSettingsStatePending
...
func (r *BMCSettingsReconciler) getBMCSettingsDifference(ctx context.Context, settings *bmcmaintenancev1alpha1.BMCSettings, bmcObj *metalv1alpha1.BMC, bmcClient bmc.BMC) (diff schemas.SettingsAttributes, err error) {
	log := ctrl.LoggerFrom(ctx)
...
func (r *BMCSettingsReconciler) checkIfMaintenanceGranted(ctx context.Context, settings *bmcmaintenancev1alpha1.BMCSettings, bmcObj *metalv1alpha1.BMC, bmcClient bmc.BMC) (bool, error) {
	log := ctrl.LoggerFrom(ctx)
	if settings.Spec.ServerMaintenanceRefs == nil {
		return false, nil
	}
...
func (r *BMCSettingsReconciler) requestMaintenanceOnServers(ctx context.Context, settings *bmcmaintenancev1alpha1.BMCSettings, bmcObj *metalv1alpha1.BMC, bmcClient bmc.BMC) (bool, error) {
	log := ctrl.LoggerFrom(ctx)
...
func (r *BMCSettingsReconciler) getBMC(ctx context.Context, settings *bmcmaintenancev1alpha1.BMCSettings) (*metalv1alpha1.BMC, error) {
	if settings.Spec.BMCRef == nil {
...
func (r *BMCSettingsReconciler) getReferredBMCSettings(ctx context.Context, referredBMCSettingsRef *corev1.LocalObjectReference) (*bmcmaintenancev1alpha1.BMCSettings, error) {
...
func (r *BMCSettingsReconciler) updateBMCSettingsStatus(ctx context.Context, settings *bmcmaintenancev1alpha1.BMCSettings, state bmcmaintenancev1alpha1.BMCSettingsState, condition *metav1.Condition) error {
	log := ctrl.LoggerFrom(ctx)
...
func (r *BMCSettingsReconciler) enqueueBMCSettingsByServerRefs(ctx context.Context, obj client.Object) []ctrl.Request {
	...
	settingsList := &bmcmaintenancev1alpha1.BMCSettingsList{}
	...
	for _, settings := range settingsList.Items {
		...
		if settings.Status.State == bmcmaintenancev1alpha1.BMCSettingsStateApplied || settings.Status.State == bmcmaintenancev1alpha1.BMCSettingsStateFailed {
			continue
		}
...
func (r *BMCSettingsReconciler) enqueueBMCSettingsByBMCVersion(ctx context.Context, obj client.Object) []ctrl.Request {
	log := ctrl.LoggerFrom(ctx)
	BMCVersion := obj.(*bmcmaintenancev1alpha1.BMCVersion)
	if BMCVersion.Status.State != bmcmaintenancev1alpha1.BMCVersionStateCompleted {
		return nil
	}
...
func (r *BMCSettingsReconciler) enqueueBMCSettingsBySecret(ctx context.Context, obj client.Object) []ctrl.Request {
	...
	list := &bmcmaintenancev1alpha1.BMCSettingsList{}
